...
/* ===============================
   1) Narzędzia fonetyczne / metryczne
   =============================== */
...
function renderHighlighted(line:string, ranges: Array<{start:number;end:number;len?:number}>) {
  if (!ranges.length) return <>{line || <span className="text-slate-400">(pusta linia)</span>}</>;
    ...
    out.push(
      <Tooltip key={`h-${idx}`}>
        <TooltipTrigger asChild>
          <span className="bg-rose-100/80 dark:bg-rose-900/40 ring-1 ring-rose-300 dark:ring-rose-700 rounded px-0.5">
            {frag}
          </span>
        </TooltipTrigger>
...
export default function App() {
  ...
  const dynamicTests=useMemo(()=>{
    ...
    return all.map(t=>{
      const found=findClusters(phonemizeLineWithMap(t.line).seq,3).length;
      const ok=Number.isFinite(t.expectClusters)? found===t.expectClusters : undefined;
      return { ...t, found, ok };
    });
  },[extraTests]);
        ...
        <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-slate-100">
          <div className="mx-auto max-w-6xl p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="dark:border-slate-700">
              ...
              <CardContent className="space-y-4">
                <Textarea rows={16} value={text} onChange={(e)=>setText(e.target.value)}
                  className="min-h-[360px] dark:bg-slate-950 dark:text-slate-100 dark:border-slate-700"
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <select value={preset} onChange={e=>setPreset(e.target.value)}
                      className="h-10 px-3 rounded-xl border dark:bg-slate-950 dark:border-slate-700"
                    >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Input type="number" min={0} max={3} value={tolerance}
                      onChange={e=>setTolerance(parseInt(e.target.value||"0",10))}
                      className="w-20 dark:bg-slate-950 dark:border-slate-700"
                    />
                  </div>
            ...
            <Card className="dark:border-slate-700">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Wand2 className="h-5 w-5"/> Analiza rytmiczna
                </CardTitle>
              </CardHeader>
                ...
                <div className="space-y-3">
                  {results.map((r,idx)=> (
                    <motion.div key={idx} initial={{opacity:0,y:6}} animate={{opacity:1,y:0}}
                      className="rounded-xl border p-3 bg-white dark:bg-slate-950 dark:border-slate-700"
                    >
                        ...
                        <div className="flex flex-col items-end gap-1 min-w-[140px]">
                          <Badge className={`border ${
                            r.meter.ok
                              ...
                              : "bg-rose-50 text-rose-700 border-rose-300 dark:bg-rose-900/30 dark:text-rose-200 dark:border-rose-700"
                          }`}>{r.syllables} syl (cel {r.meter.target ?? "–"})</Badge>
                ...
                <Card className="bg-slate-50/60 dark:bg-slate-900/40 border-dashed dark:border-slate-700">
                  <CardContent className="p-3 text-xs flex items-start gap-2">
                    ...
                    <div className="space-y-1">
                      <div><strong>Legenda:</strong> <Badge className="ml-1 mr-1">V</Badge> samogłoska, <Badge className="ml-1 mr-1">C</Badge> spółgłoska, <Badge className="ml-1 mr-1">*</Badge> akcent. Zaznaczenia w tekście = zatory (ciągi spółgłosek ≥ 3).</div>
            ...
            <Card className="lg:col-span-2 dark:border-slate-700">
              ...
              <CardContent className="space-y-3">
                <div className="flex gap-2">
                  <Input value={userTest} onChange={e=>setUserTest(e.target.value)} placeholder="Wpisz linijkę do testu…"
                ...
                <div className="grid md:grid-cols-2 gap-2">
                  {dynamicTests.map((t,idx)=> (
                    <div key={idx}
                         className="rounded-lg border p-2 text-sm bg-white dark:bg-slate-950 dark:border-slate-700 flex items-start justify-between gap-2">
                      <div>
                        <div className="font-medium">{t.line}</div>
                        <div className="text-xs text-slate-500 dark:text-slate-400">
                          zatory: {t.found}{ Number.isFinite(t.expectClusters) ? ` (oczekiwano ${t.expectClusters})` : "" }
                        </div>
                      </div>
                      {idx>=BUILT_IN_TESTS.length && (
                        <Button variant="outline" onClick={()=>rmTest(idx-BUILT_IN_TESTS.length)}
                                className="h-8 px-2 dark:border-slate-600"><Trash2 className="h-4 w-4"/></Button>
                      )}
                    </div>
                  ))}
                </div>
